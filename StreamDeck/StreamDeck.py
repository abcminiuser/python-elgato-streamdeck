...
class StreamDeck(object):
    ...
    KEY_DELAY=3000 # Delay in nanoseconds before the same key can fire again

    def __init__(self, device):
        self.device = device
        self.last_key_states = [False] * self.KEY_COUNT
        self.last_key_time = [time_ns()] * self.KEY_COUNT
            ...
            if len(payload):
                ...
                if self.key_callback is not None:
                    for k, (old, new) in enumerate(zip(self.last_key_states, new_key_states)):
                        if old != new and time_ns() > self.last_key_time[k]+self.KEY_DELAY:
                            self.last_key_time[k] = time_ns()
